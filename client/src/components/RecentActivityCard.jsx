...
import { Clock, FileText, Upload, CreditCard, Activity, Loader2, CheckCircle } from "lucide-react"
  ...
  useEffect(() => {
    const fetchRecentActivity = async () => {
      try {
        ...
        orders.slice(0, 5).forEach(order => {
          order.items.forEach(item => {
            activityList.push({
              ...
              status: order.status,
              icon: CheckCircle,
              color: order.status === 'completed' ? 'text-green-500' : 'text-amber-500'
            });
          });
        });
        ...
        setActivities(activityList.slice(0, 3)); // Show only 3 most recent
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      {activities.length === 0 ? (
        <div className="text-center text-gray-500 dark:text-gray-400 py-8">
          ...
          <p className="text-sm">Your recent print jobs will appear here</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="text-gray-900 dark:text-gray-100">{activity.description}</span>
                </div>
